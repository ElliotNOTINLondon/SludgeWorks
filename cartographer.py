...
"""
OLD SETTINGS
screen_width = 80, screen_height = 52
bar_width = int(screen_width / 4), panel_height = round(screen_height / 8), panel_y = screen_height - panel_height
message_x = int(bar_width + 2), message_width = screen_width - bar_width - 2, message_height = panel_height - 1
map_width = int(screen_width), map_height = int(screen_height - panel_height)

room_max_size = map_width / 5
room_min_size = map_width / 10
max_rooms = 50
max_monsters_per_room = 4, max_items_per_room = 2
"""
...
loot_values = {
    'heal': items.healing_potion,
    'lightning': items.lightning_scroll,
    'fireball': items.fireball_scroll,
    'confuse': items.confusion_scroll,
    'sword': items.sword,
    'helm': items.helm,
    'shield': items.shield
}
...
def _place_objects(new_map, room, player):
    ...
    # Plant dictionary
    plant_chances = {}
    plant_chances['whip_vine'] = 80

    # Monster dictionary
    monster_chances = {}
    monster_chances['wretch'] = 80
    monster_chances['hunchback'] = _from_dungeon_level(new_map, [[10, 2], [25, 4], [80, 6], [40, 10]])
    monster_chances['thresher'] = _from_dungeon_level(new_map, [[5, 4], [15, 6], [30, 8], [50, 10]])

    # Item dictionary
    item_chances = {}
    item_chances['heal'] = 35
    item_chances['lightning'] = _from_dungeon_level(new_map, [[10, 3], [25, 4]])
    item_chances['fireball'] = _from_dungeon_level(new_map, [[10, 4], [25, 6]])
    item_chances['confuse'] = _from_dungeon_level(new_map, [[10, 2]])
    item_chances['sword'] = _from_dungeon_level(new_map, [[5, 4]])
    item_chances['helm'] = _from_dungeon_level(new_map, [[5, 0], [10, 6]])
    item_chances['shield'] = _from_dungeon_level(new_map, [[5, 4], [15, 8]])
    ...
    for i in range(num_plants):
        ...
        if not new_map.is_blocked_at(pos):
            choice = _random_choice(plant_chances)
            if choice == 'whip_vine':
                fighter_component = Fighter(hp=10, xp=20, death_function=ai.monster_death,
                                            strength=3, agility=1, vitality=1, intellect=1, perception=1)
                ai_component = AI(ai.stationary, ai.BasicMonsterMetadata(player))
                monster = Object(pos, 'w', 'Whip Vine', libtcod.light_grey,
    ...
    for i in range(num_monsters):
        ...
        if not new_map.is_blocked_at(pos):
            ...
            if choice == 'wretch':
                fighter_component = Fighter(hp=10, xp=30, death_function=ai.monster_death,
                                            damage_dice=1, damage_sides=4,
                                            strength=4, agility=0, vitality=1, intellect=1, perception=1)
                ai_component = AI(ai.aggressive, ai.BasicMonsterMetadata(player))
                monster = Object(pos, 'w', 'Wretch', libtcod.darker_red,
                                 blocks=True, fighter=fighter_component, ai=ai_component)

            elif choice == 'hunchback':
                fighter_component = Fighter(hp=20, xp=75, death_function=ai.monster_death,
                                            damage_dice=2, damage_sides=6,
                                            strength=7, agility=1, vitality=1, intellect=1, perception=1)
                ai_component = AI(ai.aggressive, ai.BasicMonsterMetadata(player))
                monster = Object(pos, 'H', 'Hunchback', libtcod.brass,
                                 blocks=True, fighter=fighter_component, ai=ai_component)

            elif choice == 'thresher':
                fighter_component = Fighter(hp=50, xp=150, death_function=ai.monster_death,
                                            damage_dice=3, damage_sides=4,
                                            strength=5, agility=4, vitality=1, intellect=1, perception=1)
                ai_component = AI(ai.aggressive, ai.BasicMonsterMetadata(player))
                monster = Object(pos, 'T', 'Thresher', libtcod.dark_azure,
                                 blocks=True, fighter=fighter_component, ai=ai_component)
