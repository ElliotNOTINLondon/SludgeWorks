...
def attack(fighter, target, report=True):
    ...
    if attack_roll > defence_roll:
        damage = randint(fighter.base_damage_dice, fighter.base_damage_dice*fighter.base_damage_sides)
        if damage > 0:
            if report:
                log.message(
                    fighter.owner.name.capitalize() + ' attacks ' + target.name +
                    ' for ' + str(damage) + ' hit points.', libtcod.white)
                inflict_damage(fighter.owner, target.fighter, damage)
        elif report:
            log.message(
                fighter.owner.name.capitalize() + ' attacks ' + target.name +
                ' but does no damage.', libtcod.grey)
    else:
        log.message(
            fighter.owner.name.capitalize() + ' attacks ' + target.name +
            ' but misses.', libtcod.grey)
...
def pick_up(actor, o, report=True):
    ...
    if len(actor.inventory) >= 24:
        ...
        if equipment and _get_equipped_in_slot(actor, equipment.slot) is None:
            ...
            if report:
                log.message(actor.name.capitalize() + 'equips the ' + o.name + '!', libtcod.green)
        return True
