...
_con = None
# Buffer overlaid over the main console window for effects, labels, and other metadata
_overlay = None
# UI text data
_panel = None
...
def renderer_init():
    ...
    libtcod.console_init_root(config.SCREEN_WIDTH, config.SCREEN_HEIGHT, 'SludgeWorks', False)
...
def parse_move(key):
    ...
    elif key.vk == libtcod.KEY_KP7 or key_char == 'y' or key_char == 'Y':
        return True, algebra.northwest, key.shift
    elif key.vk == libtcod.KEY_KP9 or key_char == 'u' or key_char == 'U':
        return True, algebra.northeast, key.shift
    elif key.vk == libtcod.KEY_KP1 or key_char == 'b' or key_char == 'B':
        return True, algebra.southwest, key.shift
    elif key.vk == libtcod.KEY_KP3 or key_char == 'n' or key_char == 'N':
...
def main_menu(new_game, play_game, load_game):
    ...
    while not libtcod.console_is_window_closed():
        ...
        libtcod.console_print_ex(
            0, int(config.SCREEN_WIDTH / 2), int(config.SCREEN_HEIGHT / 2) - 4, libtcod.BKGND_NONE,
            libtcod.CENTER, 'SludgeWorks')
        libtcod.console_print_ex(
            0, int(config.SCREEN_WIDTH / 2), int(config.SCREEN_HEIGHT - 2), libtcod.BKGND_NONE,
            libtcod.CENTER, 'designed by the Supreme Peasant')

        (char, choice) = menu('', ['New Game', 'Continue', 'Quit'], 24)
...
def _render_bar(x, y, total_width, name, value, maximum, bar_color, back_color):
    ...
    libtcod.console_print_ex(
        _panel, int(x + total_width / 2), y, libtcod.BKGND_NONE, libtcod.CENTER,
...
def _get_names_under_mouse(player, pointer_location):
    (sx, sy) = pointer_location
    if (sx < 0 or sy < 0 or
